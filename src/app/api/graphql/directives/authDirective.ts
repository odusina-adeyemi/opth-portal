      ...
      return mapSchema(schema, {
        [MapperKind.OBJECT_FIELD]: (fieldConfig, _fieldName, typeName) => {
          ...
          fieldConfig.resolve = function (source, args, context, info) {
            ...
            const allowedActionsForOptometrist = ['createPatient'];

            // Allow optometrist role to perform specific actions
            if (
              context.user.role === 'optometrist' &&
              allowedActionsForOptometrist.includes(info.fieldName)
            ) {
              return resolve(source, args, context, info);
            }

            // Restrict all other roles/actions
